(() => {
  ...
  const titles = {
    ...
    users: ['Pengguna', 'Kelola akun Admin dan Guru serta hak akses aplikasi.']
  };
  ...
  async function init() {
    bindNavigation(); bindForm(); bindStudents(); bindTeachers(); bindHistory(); bindHomeroom(); bindStudentProfile(); bindSettings(); bindModal(); bindUsers(); bindLogin();
  ...
  function isStudentRole() { return String(state.auth.user?.role||'') === 'Siswa/Wali'; }
  ...
  function normalizeRole(v){const s=String(v||'').trim().toLowerCase();if(s==='admin')return'Admin';if(s==='guru')return'Guru';return'';}
