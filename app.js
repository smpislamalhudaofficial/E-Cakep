(() => {
  ...
  const API_VERSION = Number(window.ECAKEP_CONFIG?.API_VERSION || 30);
  ...
  const titles = {
    ...
    users: ['Akun Pengguna', 'Kelola login Admin, Guru, serta Siswa/Wali Murid.'],
    studentportal: ['Siswa / Wali Murid', 'Lihat poin dan riwayat kedisiplinan siswa secara pribadi.'],
    settings: ['Pengaturan', 'Atur koneksi E-CAKEP dengan Google Spreadsheet.']
  };
  ...
  async function init() {
    bindLogin(); bindNavigation(); bindForm(); bindStudents(); bindTeachers(); bindUsers(); bindHistory(); bindHomeroom(); bindStudentProfile(); bindStudentPortal(); bindSettings(); bindModal();
    updateClock(); setInterval(updateClock, 1000);
    refreshAll();
    await loadCentralServerConfig();
    if (state.session?.user && state.server.configured && !state.session?.token) {
      state.session = null;
      sessionStorage.removeItem(STORAGE.session);
      $('#loginStatus').textContent = 'Google Apps Script aktif. Login kembali untuk mengaktifkan sesi.';
    }
    if (state.session?.user) enterApp(false); else showLogin();
  }
  ...
  async function login(e) {
    ...
    try {
      if (!state.server.configured) throw new Error('URL Google Apps Script belum diatur pada config.js. Hubungi Admin E-CAKEP.');
      ...
      if (!json.ok || !json.user || !json.token) throw new Error(json.message || 'Login server gagal.');
      const session = { user: json.user, token: json.token, online: true };
      state.session = session; saveSession(session);
      enterApp(true);
      await syncFromServer(false);
      toast('Login berhasil', `Selamat datang, ${json.user.nama || json.user.username}.`, 'success');
    } catch (err) {
      state.session = null;
      sessionStorage.removeItem(STORAGE.session);
      toast('Login gagal', err.message || 'Periksa username dan password.', 'error');
      $('#loginStatus').textContent = err.message || 'Login gagal.';
    } finally { btn.disabled = false; btn.textContent = 'Masuk'; }
  }

  function logout() {
    stopQrScanner(); state.session = null; sessionStorage.removeItem(STORAGE.session); state.students=migrateLocalStudents(load(STORAGE.students,[])); state.teachers=load(STORAGE.teachers,[]); state.violations=migrateLocalViolations(load(STORAGE.violations,[])); state.users=load(STORAGE.users,[DEFAULT_ADMIN]); refreshAll(); showLogin();
  }

  function showLogin() {
    $('#appShell').classList.add('hidden'); $('#loginScreen').classList.remove('hidden');
    $('#loginPassword').value = ''; $('#loginUsername').focus();
    $('#loginStatus').textContent = state.server.configured ? 'Terhubung ke Google Apps Script. Silakan login.' : 'Aplikasi belum dikonfigurasi. Admin perlu mengisi APPS_SCRIPT_URL pada config.js.';
  }

  function enterApp(showWelcome) {
    $('#loginScreen').classList.add('hidden'); $('#appShell').classList.remove('hidden');
    const u = currentUser();
    $('#currentUserName').textContent = u?.nama || u?.username || '-'; $('#currentUserRole').textContent = u?.role || '-';
    renderCurrentUserAvatar();
    applyRoleUi(); switchView(isStudentRole() ? 'studentportal' : 'dashboard');
    updateSyncUi(Boolean(state.server.configured && state.session?.online), state.server.configured ? (state.session?.online ? 'Terhubung' : 'GitHub siap') : 'Belum dikonfigurasi');
    if (showWelcome) toast('Selamat datang', `${u?.nama || u?.username} • ${u?.role}`, 'success');
  }

  function currentUser() { return state.session?.user || null; }
  function isAdmin() { return String(currentUser()?.role || '').toLowerCase() === 'admin'; }
  function isStudentRole() { return ['siswa/wali','siswa','wali'].includes(String(currentUser()?.role || '').toLowerCase()); }
  function isStaff() { return !isStudentRole(); }
  function linkedStudent() { const id=currentUser()?.studentId||''; return state.students.find(s=>s.id===id)||null; }
  ...
  function switchView(view) {
    if (isStudentRole()) view = 'studentportal';
    else if (!isAdmin() && ['teachers','users','settings','studentportal'].includes(view)) view = 'dashboard';
    ...
    $('#pageTitle').textContent = titles[view][0]; $('#pageSubtitle').textContent = titles[view][1]; closeSidebar();
    if (view === 'dashboard') renderDashboard(); if (view === 'homeroom') renderHomeroom(); if (view === 'history') renderHistory();
    if (view === 'students') renderStudents(); if (view === 'studentprofile') renderStudentProfile(); if (view === 'teachers') renderTeachers(); if (view === 'users') renderUsers(); if (view === 'studentportal') renderStudentPortal(); if (view === 'settings') renderSettingsStatus();
  }
  ...
  function bindSettings() {
    $('#saveSettingsBtn').addEventListener('click', () => {
      toast('Konfigurasi GitHub Pages', 'URL Apps Script diatur satu kali pada file config.js sebelum dipublikasikan ke GitHub Pages.', 'error');
    });
  ...
  function refreshAll(){ renderStudentOptions();renderTeacherOptions();renderDashboard();renderStudentClassFilter();renderStudents();renderTeachers();renderUsers();renderHistoryFilters();renderHistory();renderHomeroomClassOptions();renderHomeroom();renderStudentProfile();renderStudentPortal();renderCurrentUserAvatar();renderSettingsStatus();applyRoleUi(); ;setTimeout(()=>hydrateProfilePhotos(document),0);}
  ...
  async function hydrateProfilePhotos(root=document){
    ...
    try{
      for(let i=0;i<keys.length;i+=25){
        ...
        const res=await apiPost('getProfilePhotos',{items:batch},true), json=await parseApiJson(res);
  ...
  function refreshPhotoViews(){
    renderStudents();renderTeachers();renderStudentProfile();renderStudentPortal();renderCurrentUserAvatar();renderUsers();
  ...
  async function uploadProfilePhoto(type,id,file,showToast=true){
    ...
    try{
      if(!state.session?.token)throw new Error('Sesi Admin belum aktif.');
      const imageData=await fileToCompressedJpeg(file);
      const res=await apiPost('uploadProfilePhoto',{type,id,imageData},true),json=await parseApiJson(res);
  ...
  async function deleteProfilePhoto(type,id){
    ...
    try{
      const res=await apiPost('deleteProfilePhoto',{type,id},true),json=await parseApiJson(res);
  ...
  async function loadCentralServerConfig(){
    if (!state.server || typeof state.server !== 'object') state.server = { configured:false, editable:false, apiUrl:'' };
    const url=normalizeApiUrl(window.ECAKEP_CONFIG?.APPS_SCRIPT_URL||'');
    const valid=/^https:\/\/script\.google\.com\/macros\/s\/[^/]+\/exec$/i.test(url) && !url.includes('PASTE_URL');
    state.server={configured:valid,editable:false,apiUrl:valid?url:''};
    const input=$('#apiUrl');if(input){input.value=valid?url:'';input.disabled=true;input.placeholder='Diatur melalui config.js';}
    const saveBtn=$('#saveSettingsBtn');if(saveBtn){saveBtn.disabled=true;saveBtn.textContent='Diatur di config.js';}
    renderSettingsStatus();
    updateSyncUi(Boolean(valid&&state.session?.online),valid?(state.session?.online?'Terhubung':'GitHub siap'):'Belum dikonfigurasi');
    return state.server;
  }

  async function saveCentralServerConfig(){throw new Error('Pada GitHub Pages, URL Apps Script diatur melalui file config.js.');}

  async function testConnection(){
    const btn=$('#testConnectionBtn'),original=btn.textContent;btn.disabled=true;btn.textContent='Menguji...';
    try{
      if(!state.server.configured)throw new Error('APPS_SCRIPT_URL belum diisi pada config.js.');
      ...
      updateSyncUi(Boolean(state.session?.token),'Web App aktif');
      toast('Koneksi berhasil',`${json.app||'E-CAKEP'} terhubung. Spreadsheet dan Google Drive siap digunakan.`,'success');
    }catch(err){updateSyncUi(false,'Gagal terhubung');toast('Koneksi gagal',err.message||'Periksa config.js dan deployment Apps Script.','error');}
    finally{btn.disabled=false;btn.textContent=original;}
  }

  async function restoreLocalSnapshotToServer(){
    if(!isAdmin())return toast('Akses ditolak','Pemulihan data hanya dapat dilakukan Admin.','error');
    if(!state.server.configured)return toast('Belum dikonfigurasi','Isi APPS_SCRIPT_URL pada config.js.','error');
    if(!state.session?.token)return toast('Sesi belum aktif','Login kembali sebagai Admin.','error');
    const ok=confirm(`Pulihkan data lokal ke Google Spreadsheet?\n\nData E-CAKEP pada Spreadsheet akan diganti dengan data yang saat ini tersimpan di browser ini.\n\nSiswa: ${state.students.length}\nGuru: ${state.teachers.length}\nPelanggaran: ${state.violations.length}\nAkun: ${state.users.length}`);
    if(!ok)return;
    ...
    try{
      const users=state.users.map(u=>({...sanitizeUser(u),passwordHash:u.passwordHash||''}));
      const res=await pushAction('restoreLocalSnapshot',{data:{students:state.students,teachers:state.teachers,violations:state.violations,users}},true);
  ...
  async function syncFromServer(showToast=true){
    if(!state.server.configured){if(showToast)toast('Belum dikonfigurasi','Isi APPS_SCRIPT_URL pada config.js.','error');return;}
    if(!state.session?.token){if(showToast)toast('Sesi belum aktif','Silakan login kembali.','error');return;}
    if(state.syncing)return;state.syncing=true;setSyncingUi(true);
    try{
      const res=await apiGet('getAll',state.session.token),json=await parseApiJson(res);
      if(!json.ok)throw new Error(json.message||'Gagal membaca data server.');
      if(Number(json.apiVersion||0)<API_VERSION)throw new Error('Backend Apps Script masih versi lama.');
      ...
      if(Array.isArray(json.users)&&isAdmin())state.users=json.users.map(u=>({...u,passwordHash:''}));
      persistCore();save(STORAGE.users,state.users);state.photoCache.clear();refreshAll();
      state.session.online=true;saveSession(state.session);updateSyncUi(true,'Terhubung');setTimeout(()=>hydrateProfilePhotos(document),0);
      if(showToast)toast('Sinkronisasi selesai','Data terbaru dari Google Spreadsheet telah dimuat.','success');
    }catch(err){state.session.online=false;saveSession(state.session);updateSyncUi(false,'Server gagal');if(showToast)toast('Sinkronisasi gagal',err.message||'Coba kembali.','error');}
  ...
  function bridgeRequest(action,payload={},token=''){
    return new Promise((resolve,reject)=>{
      if(!state.server.configured)return reject(new Error('APPS_SCRIPT_URL belum dikonfigurasi pada config.js.'));
  ...
  async function apiPost(action,payload,withToken=true){return responseLike(await bridgeRequest(action,payload,withToken?(state.session?.token||''):''));}
  ...
  function renderSettingsStatus(){
    ...
    const has=Boolean(state.server.configured),active=Boolean(state.session?.token);
    $('#settingsStatus').innerHTML=`<span class="status-dot ${has&&active?'online':'offline'}"></span><div><strong>${has?(active?'GitHub Pages terhubung':'Konfigurasi GitHub siap'):'Belum dikonfigurasi'}</strong><p>${has?(active?'Data tersinkron langsung ke Google Spreadsheet; foto profil disimpan di Google Drive.':'URL Apps Script dibaca otomatis dari config.js. Login untuk mengaktifkan sesi.'):'Edit file config.js dan isi APPS_SCRIPT_URL dengan URL deployment /exec.'}</p></div>`;
  }
