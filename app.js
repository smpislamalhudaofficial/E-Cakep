(() => {
  ...
  async function hydrateProfilePhotos(root=document){
    ...
    try{
      for(let i=0;i<keys.length;i+=25){
        ...
        const res=await apiPost('getProfilePhotos',{items:batch}), json=assertNoLoginBackend(await parseApiJson(res));
  ...
  async function uploadProfilePhoto(type,id,file,showToast=true){
    ...
    try{
      ...
      const res=await apiPost('uploadProfilePhoto',{type,id,imageData}),json=assertNoLoginBackend(await parseApiJson(res));
  ...
  async function deleteProfilePhoto(type,id){
    ...
    try{
      const res=await apiPost('deleteProfilePhoto',{type,id}),json=assertNoLoginBackend(await parseApiJson(res));
  ...
  function assertNoLoginBackend(json){
    const msg=String(json?.message||'');
    if(/sesi|session|login|token|autentikasi|authentication/i.test(msg)){
      throw new Error('Backend Google Apps Script yang aktif masih memakai sistem login/sesi. Ganti Code.gs dengan v3.2 TANPA LOGIN, jalankan setupSheets(), lalu Deploy > Manage deployments > Edit > New version > Deploy.');
    }
    if(Number(json?.apiVersion||0)!==API_VERSION || String(json?.authMode||'').toUpperCase()!=='NONE'){
      throw new Error(`Backend belum v3.2 Tanpa Login. Ditemukan API ${json?.apiVersion||0}${json?.authMode?` / auth ${json.authMode}`:''}; dibutuhkan API ${API_VERSION} / auth NONE. Deploy ulang Code.gs v3.2 sebagai New version.`);
    }
    return json;
  }
  ...
  async function testConnection(){
    ...
    try{
      ...
      const res=await apiPost('ping',{}),json=assertNoLoginBackend(await parseApiJson(res));
  ...
  async function syncFromServer(showToast=true){
    ...
    try{
      const res=await apiPost('getAll',{}),json=assertNoLoginBackend(await parseApiJson(res));
  ...
  function bridgeRequest(action,payload={}){
    return new Promise((resolve,reject)=>{
      ...
      const fields={action,payload:JSON.stringify(payload||{}),transport:'iframe',requestId,origin:location.origin};
