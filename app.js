(() => {
  ...
  function canView(view){ if(isAdmin())return true; if(isStudentRole())return view==='studentportal'; return ['dashboard','homeroom','history','students','studentprofile'].includes(view); }
  ...
  function showAccountProfile(){
    const u=currentUser();if(!u)return;
    const s=linkedStudent();const t=u.teacherId?state.teachers.find(x=>x.id===u.teacherId):null;
    const lines=[];
    lines.push(`<div class="profile-account-card">${avatarHtml(u.role==='Guru'?'teacher':u.role==='Siswa'?'student':'',u.role==='Guru'?t?.id:u.role==='Siswa'?s?.id:'',u.nama,'profile-avatar-lg')}<div><span class="eyebrow">PROFIL PENGGUNA</span><h3>${escHtml(u.nama)}</h3><p>${escHtml(u.role)} • @${escHtml(u.username)}</p></div></div>`);
    if(t)lines.push(`<div class="profile-info-grid"><div class="profile-info-item"><span>Jabatan</span><strong>${escHtml(t.jabatan||'-')}</strong></div><div class="profile-info-item"><span>Alamat</span><strong>${escHtml(t.alamat||'-')}</strong></div><div class="profile-info-item"><span>Wali Kelas</span><strong>${escHtml(t.waliKelas||'Bukan wali kelas')}</strong></div></div>`);
    if(s){const wali=getHomeroomTeacher(s.kelas);lines.push(`<div class="profile-info-grid"><div class="profile-info-item"><span>NISN</span><strong>${escHtml(s.nisn)}</strong></div><div class="profile-info-item"><span>Kelas</span><strong>${escHtml(s.kelas)}</strong></div><div class="profile-info-item"><span>Wali Kelas</span><strong>${wali?escHtml(wali.nama):'Belum ditentukan'}</strong></div></div>`);}
    const teacherHistory=u.role==='Guru'?state.violations.filter(v=>v.teacherId===u.teacherId).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt))):[];
    const printBtn=u.role==='Siswa'?'<button class="btn secondary" type="button" data-profile-print-account>Cetak Profil</button>':'';
    const historyMarkup=u.role==='Guru'?`<div class="profile-section-head"><div><span class="eyebrow">RIWAYAT PENCATATAN</span><h3>Catatan yang saya buat</h3></div></div><div class="table-wrap"><table><thead><tr><th>Tanggal</th><th>Siswa</th><th>Kelas</th><th>Pelanggaran</th></tr></thead><tbody data-my-teacher-history></tbody></table></div><div class="pagination" data-my-teacher-pagination></div>`:'';
    const box=document.createElement('div');box.className='modal-backdrop';
    box.innerHTML=`<div class="modal teacher-profile-modal"><div class="modal-head"><div><span class="eyebrow">AKUN</span><h3>Profil Saya</h3></div><button class="icon-btn" type="button">✕</button></div><div class="modal-body">${lines.join('')}${historyMarkup}<div class="modal-actions">${printBtn}<button class="btn primary" type="button" data-close-profile>Tutup</button></div></div></div>`;
    document.body.appendChild(box);box.querySelector('.icon-btn').onclick=()=>box.remove();box.querySelector('[data-close-profile]').onclick=()=>box.remove();
    box.querySelector('[data-profile-print-account]')?.addEventListener('click',()=>{if(s)printStudentProfile(s.id);});
    if(u.role==='Guru'){
      const body=box.querySelector('[data-my-teacher-history]'),pager=box.querySelector('[data-my-teacher-pagination]');let page=1;const per=20;
      const draw=()=>{const total=Math.max(1,Math.ceil(teacherHistory.length/per));page=Math.min(page,total);const slice=teacherHistory.slice((page-1)*per,page*per);body.innerHTML=slice.map(v=>`<tr><td>${escHtml(displayDate(v.tanggal))}<br><span class="recent-meta">${escHtml(v.waktu)}</span></td><td>${escHtml(v.nama)}</td><td>${escHtml(v.kelas)}</td><td>${escHtml(v.jenisPelanggaran)}</td></tr>`).join('')||`<tr><td colspan="4" class="muted">Belum ada catatan.</td></tr>`;pager.innerHTML=`<button class="page-btn" ${page<=1?'disabled':''} data-prev>‹</button><span>Halaman ${page} dari ${total}</span><button class="page-btn" ${page>=total?'disabled':''} data-next>›</button>`;pager.querySelector('[data-prev]')?.addEventListener('click',()=>{page--;draw();});pager.querySelector('[data-next]')?.addEventListener('click',()=>{page++;draw();});};draw();
    }
  }
  ...
  async function saveEntity(e) {
    ...
    if (state.modalMode === 'student') {
      ...
      const existing=state.teachers.find(x=>x.id===state.editingId); const item={id:state.editingId||uid(),nama:String(fd.get('nama')).trim(),alamat:String(fd.get('alamat')).trim(),jabatan:String(fd.get('jabatan')).trim(),waliKelas:String(fd.get('waliKelas')||'').trim(),updatedAt:now,photoId:existing?.photoId||''}; const photoFile=e.currentTarget.querySelector('input[name="foto"]')?.files?.[0]||null; upsertLocal(state.teachers,item); state.teachers.sort(sortByName); save(STORAGE.teachers,state.teachers); await pushAction('saveTeacher',{data:item},true); if(photoFile) await uploadProfilePhoto('teacher',item.id,photoFile,false); toast('Data guru disimpan',item.nama,'success');
    } else {
  ...
  function confirmDialog(title,message,confirmText='Ya, lanjutkan',danger=true){
    return new Promise(resolve=>{
      const box=document.createElement('div');box.className='confirm-backdrop';
      box.innerHTML=`<div class="confirm-modal" role="dialog" aria-modal="true"><div class="confirm-icon">!</div><h3>${escHtml(title)}</h3><p>${escHtml(message)}</p><div class="confirm-actions"><button class="btn secondary" data-cancel>Batal</button><button class="btn ${danger?'danger':'primary'}" data-ok>${escHtml(confirmText)}</button></div></div>`;
      document.body.appendChild(box);const close=v=>{box.remove();resolve(v);};box.querySelector('[data-cancel]').onclick=()=>close(false);box.querySelector('[data-ok]').onclick=()=>close(true);box.addEventListener('click',e=>{if(e.target===box)close(false);});
    });
  }
  async function deleteStudent(id){ if(!isAdmin())return; const s=state.students.find(x=>x.id===id); if(!s||!(await confirmDialog('Hapus Data Siswa?',`Data ${s.nama} akan dihapus. Riwayat pelanggaran tidak ikut dihapus.`,'Hapus')))return; state.students=state.students.filter(x=>x.id!==id); save(STORAGE.students,state.students); refreshAll(); await pushAction('deleteStudent',{id},false); toast('Data siswa dihapus',s.nama,'success'); }
  async function deleteTeacher(id){ if(!isAdmin())return; const t=state.teachers.find(x=>x.id===id); if(!t||!(await confirmDialog('Hapus Data Guru?',`Data ${t.nama} akan dihapus.`,'Hapus')))return; state.teachers=state.teachers.filter(x=>x.id!==id); save(STORAGE.teachers,state.teachers); refreshAll(); await pushAction('deleteTeacher',{id},false); toast('Data guru dihapus',t.nama,'success'); }
  async function deleteUser(id){ if(!isAdmin())return; const u=state.users.find(x=>x.id===id); if(!u)return; if(u.id===currentUser()?.id)return toast('Tidak dapat menghapus akun aktif','Logout dan gunakan akun admin lain.','error'); if(!(await confirmDialog('Hapus Akun?',`Akun ${u.username} akan dihapus.`,'Hapus')))return; state.users=state.users.filter(x=>x.id!==id); save(STORAGE.users,state.users); refreshAll(); await pushAction('deleteUser',{id},false); toast('Akun dihapus',u.username,'success'); }

  async function deleteViolation(id){ if(!isAdmin()) return toast('Akses ditolak','Hanya Admin yang dapat menghapus riwayat pelanggaran.','error'); const v=state.violations.find(x=>x.id===id); if(!v||!(await confirmDialog('Hapus Catatan Pelanggaran?',`Catatan ${v.nama} akan dihapus dan poin dapat dikembalikan jika sesuai riwayat reset.`,'Hapus')))return; const s=state.students.find(x=>x.id===v.studentId); if(s && (!s.resetAt || String(v.createdAt)>=String(s.resetAt))) s.poin=Math.min(100,normalizePoint(s.poin)+Number(v.pointPotongan||pointFor(v.klasifikasi))); state.violations=state.violations.filter(x=>x.id!==id); persistCore(); refreshAll(); const res=await pushAction('deleteViolation',{id},false); if(res?.student)updateStudentFromServer(res.student); toast('Catatan dihapus',v.nama,'success'); }

  async function resetStudentPoints(id){ if(!isAdmin())return; const s=state.students.find(x=>x.id===id); if(!s||!(await confirmDialog('Reset Poin Siswa?',`Poin ${s.nama} akan dikembalikan menjadi 100. Riwayat tetap disimpan.`,'Reset')))return; const now=new Date().toISOString(); s.poin=100;s.resetAt=now;s.updatedAt=now;save(STORAGE.students,state.students);refreshAll();const res=await pushAction('resetStudentPoints',{id},false);if(res?.student)updateStudentFromServer(res.student);toast('Poin direset',`${s.nama} kembali ke 100 poin.`,'success'); }
  async function resetAllPoints(){ if(!isAdmin()||!state.students.length)return; if(!(await confirmDialog('Reset Semua Poin?',`Poin seluruh ${state.students.length} siswa akan menjadi 100. Riwayat pelanggaran tetap disimpan.`,'Reset Semua')))return; const now=new Date().toISOString(); state.students.forEach(s=>{s.poin=100;s.resetAt=now;s.updatedAt=now;});save(STORAGE.students,state.students);refreshAll();await pushAction('resetAllStudentPoints',{},false);toast('Semua poin direset','Seluruh siswa kembali ke 100 poin.','success'); }
  ...
  function visibleViolations(){ const u=currentUser(); if(!u||isAdmin())return state.violations; if(isStudentRole())return state.violations.filter(v=>v.studentId===u.studentId||(!v.studentId&&v.nisn===linkedStudent()?.nisn)); if(u.role==='Guru')return state.violations; return state.violations; }
  ...
  function renderTeachers(){ const q=($('#teacherSearch')?.value||'').toLowerCase().trim();const rows=[...state.teachers].sort(sortByName).filter(t=>[t.nama,t.alamat,t.jabatan,t.waliKelas].join(' ').toLowerCase().includes(q));$('#teacherCount').textContent=`${rows.length} guru`;$('#teacherTableBody').innerHTML=rows.map(t=>`<tr><td>${avatarHtml('teacher',t.id,t.nama,'avatar-sm')}</td><td><button class="link-button teacher-name-link" data-open-teacher-profile="${escAttr(t.id)}">${escHtml(t.nama)}</button></td><td>${escHtml(t.alamat)}</td><td>${escHtml(t.jabatan)}</td><td>${escHtml(t.waliKelas||'-')}</td><td><div class="row-actions">${isAdmin()?`<button class="action-btn edit" data-edit-teacher="${escAttr(t.id)}">Ubah</button><button class="action-btn delete" data-delete-teacher="${escAttr(t.id)}">Hapus</button>`:''}</div></td></tr>`).join('');$('#teacherEmpty').classList.toggle('hidden',rows.length>0);$$('[data-open-teacher-profile]').forEach(b=>b.onclick=()=>showTeacherProfile(b.dataset.openTeacherProfile));$$('[data-edit-teacher]').forEach(b=>b.onclick=()=>openEntityModal('teacher',b.dataset.editTeacher));$$('[data-delete-teacher]').forEach(b=>b.onclick=()=>deleteTeacher(b.dataset.deleteTeacher)); }
  function showTeacherProfile(id){
    const t=state.teachers.find(x=>x.id===id); if(!t)return;
    const rows=state.violations.filter(v=>v.teacherId===id || String(v.guru||'').trim().toLowerCase()===String(t.nama||'').trim().toLowerCase()).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));
    const box=document.createElement('div'); box.className='modal-backdrop';
    box.innerHTML=`<div class="modal teacher-profile-modal"><div class="modal-head"><div><span class="eyebrow">PROFIL GURU</span><h3>${escHtml(t.nama)}</h3></div><button class="icon-btn" type="button" data-close>✕</button></div><div class="modal-body"><div class="profile-account-card">${avatarHtml('teacher',t.id,t.nama,'profile-avatar-lg')}<div><h3>${escHtml(t.nama)}</h3><p>${escHtml(t.jabatan||'-')}</p></div></div><div class="profile-info-grid"><div class="profile-info-item"><span>Alamat</span><strong>${escHtml(t.alamat||'-')}</strong></div><div class="profile-info-item"><span>Wali Kelas</span><strong>${escHtml(t.waliKelas||'Bukan wali kelas')}</strong></div><div class="profile-info-item"><span>Total Catatan</span><strong>${rows.length}</strong></div></div><div class="profile-section-head"><div><span class="eyebrow">RIWAYAT PENCATATAN</span><h3>20 catatan per halaman</h3></div></div><div class="table-wrap"><table><thead><tr><th>Tanggal</th><th>Siswa</th><th>Kelas</th><th>Pelanggaran</th><th>Poin</th></tr></thead><tbody data-teacher-history></tbody></table></div><div class="pagination" data-teacher-pagination></div><div class="modal-actions"><button class="btn primary" type="button" data-close>Tutup</button></div></div></div>`;
    document.body.appendChild(box);
    const body=box.querySelector('[data-teacher-history]'), pager=box.querySelector('[data-teacher-pagination]');
    let page=1; const per=20;
    const draw=()=>{const total=Math.max(1,Math.ceil(rows.length/per));page=Math.min(page,total);const slice=rows.slice((page-1)*per,page*per);body.innerHTML=slice.map(v=>`<tr><td>${escHtml(displayDate(v.tanggal))}<br><span class="recent-meta">${escHtml(v.waktu)}</span></td><td><strong>${escHtml(v.nama)}</strong></td><td>${escHtml(v.kelas)}</td><td>${escHtml(v.jenisPelanggaran)}</td><td>-${Number(v.pointPotongan||pointFor(v.klasifikasi))}</td></tr>`).join('')||`<tr><td colspan="5" class="muted">Belum ada catatan.</td></tr>`;pager.innerHTML=`<button class="page-btn" ${page<=1?'disabled':''} data-prev>‹</button><span>Halaman ${page} dari ${total}</span><button class="page-btn" ${page>=total?'disabled':''} data-next>›</button>`;pager.querySelector('[data-prev]')?.addEventListener('click',()=>{page--;draw();});pager.querySelector('[data-next]')?.addEventListener('click',()=>{page++;draw();});};
    box.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>box.remove()); draw(); setTimeout(()=>hydrateProfilePhotos(box),0);
  }

  function renderUsers(){
    if(!$('#userTableBody'))return;
    const q=($('#userSearch')?.value||'').toLowerCase().trim();
    const rows=[...state.users].filter(u=>[u.username,u.nama,u.role,u.kelas].join(' ').toLowerCase().includes(q));
    $('#userCount').textContent=`${rows.length} akun`;
    $('#userTableBody').innerHTML=rows.map(u=>{
      const t=state.teachers.find(x=>x.id===u.teacherId),st=state.students.find(x=>x.id===u.studentId);
      const photo=u.role==='Guru'?avatarHtml('teacher',t?.id||'',t?.nama||u.nama,'avatar-sm'):u.role==='Siswa'?avatarHtml('student',st?.id||'',st?.nama||u.nama,'avatar-sm'):avatarHtml('','',u.nama,'avatar-sm');
      const pw=u.password?`<span class="password-wrap"><span class="password-text" data-password-value="${escAttr(u.password)}">••••••••</span><button type="button" class="password-eye" data-show-password="${escAttr(u.id)}" title="Tampilkan password" aria-label="Tampilkan password">◉</button></span>`:'<span class="muted">Belum tersedia</span>';
      return `<tr><td>${photo}</td><td><strong>${escHtml(u.username)}</strong></td><td>${escHtml(u.nama)}</td><td><span class="role-badge">${escHtml(u.role)}</span></td><td>${escHtml(t?.nama||'-')}</td><td>${st?`${escHtml(st.nama)}<br><span class="recent-meta">${escHtml(st.nisn)} • ${escHtml(st.kelas)}</span>`:'-'}</td><td>${escHtml(u.kelas||t?.waliKelas||'-')}</td><td>${pw}</td><td>${u.aktif===false?'<span class="status-badge off">Nonaktif</span>':'<span class="status-badge on">Aktif</span>'}</td><td><div class="row-actions"><button class="action-btn edit" data-edit-user="${escAttr(u.id)}">Ubah</button><button class="action-btn delete" data-delete-user="${escAttr(u.id)}">Hapus</button></div></td></tr>`;
    }).join('');
    $$('[data-show-password]').forEach(b=>b.onclick=()=>{const u=state.users.find(x=>x.id===b.dataset.showPassword);const el=b.parentElement.querySelector('[data-password-value]');if(!u||!el)return;const shown=el.dataset.visible==='1';el.dataset.visible=shown?'0':'1';el.textContent=shown?'••••••••':u.password;b.textContent=shown?'◉':'◉';});
    $$('[data-edit-user]').forEach(b=>b.onclick=()=>openEntityModal('user',b.dataset.editUser));
  ...
  function selectedHomeroomClass(){ const u=currentUser(); const chosen=$('#homeroomClassSelect')?.value||''; return (isAdmin()||u?.role==='Guru') ? (chosen||u?.kelas||getClasses()[0]||'') : ''; }
  ...
  function renderStudentPortal(){if(!$('#portalHistoryBody'))return;const s=linkedStudent();if(!isStudentRole())return;if(!s){$('#portalStudentName').textContent='Menu Siswa / Wali Murid';setAvatarElement($('#portalStudentAvatar'),'student','', 'S');$('#portalIdentity').innerHTML='<div class="info-box">Akun ini belum ditautkan ke Data Siswa. Hubungi Admin E-CAKEP.</div>';$('#portalHistoryBody').innerHTML='';$('#portalEmpty').classList.add('hidden');$('#downloadMyRecapBtn').disabled=true;return;}$('#downloadMyRecapBtn').disabled=false;$('#portalStudentName').textContent=s.nama;setAvatarElement($('#portalStudentAvatar'),'student',s.id,s.nama);const wali=getHomeroomTeacher(s.kelas);$('#portalIdentity').innerHTML=`<div class="portal-id-item"><span>Nama</span><strong>${escHtml(s.nama)}</strong></div><div class="portal-id-item"><span>NISN</span><strong>${escHtml(s.nisn)}</strong></div><div class="portal-id-item"><span>Jenis Kelamin</span><strong>${escHtml(s.jenisKelamin)}</strong></div><div class="portal-id-item"><span>Kelas</span><strong>${escHtml(s.kelas)}</strong></div><div class="portal-id-item"><span>Wali Kelas</span><strong>${wali?`Wali Kelas ${escHtml(wali.nama)}`:'Belum ditentukan'}</strong></div>`;const vv=state.violations.filter(v=>v.studentId===s.id||(!v.studentId&&v.nisn===s.nisn)).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));$('#portalPoint').textContent=normalizePoint(s.poin);$('#portalViolationCount').textContent=vv.length;$('#portalDeduction').textContent=vv.reduce((n,v)=>n+Number(v.pointPotongan||pointFor(v.klasifikasi)),0);$('#portalLastDate').textContent=vv[0]?displayDate(vv[0].tanggal):'-';$('#portalHistoryBody').innerHTML=vv.map(v=>`<tr><td><strong>${escHtml(displayDate(v.tanggal))}</strong><br><span class="recent-meta">${escHtml(v.waktu)}</span></td><td>${escHtml(v.guru)}</td><td><span class="classification ${classificationClass(v.klasifikasi)}">${escHtml(v.klasifikasi)}</span></td><td>-${Number(v.pointPotongan||pointFor(v.klasifikasi))}</td><td>${escHtml(v.sisaPoin??'-')}</td><td>${escHtml(v.jenisPelanggaran)}</td><td>${escHtml(v.keterangan||'-')}</td></tr>`).join('');$('#portalEmpty').classList.toggle('hidden',vv.length>0);}
  ...
  async function printStudentProfile(id){
    const s=state.students.find(x=>x.id===id);if(!s)return;
    if(s.photoId&&!state.photoCache.has(photoKey('student',id))&&state.server.configured){
      try{const res=await apiPost('getProfilePhotos',{items:[{type:'student',id}]}),json=assertBackend(await parseApiJson(res));const v=json.photos?.[photoKey('student',id)];if(v)state.photoCache.set(photoKey('student',id),v);}catch(_){}
    }
    const rows=state.violations.filter(v=>v.studentId===id||v.nisn===s.nisn).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));
    const html=studentProfilePrintHtml(s,rows);const w=window.open('','_blank','width=1000,height=800');if(!w)return toast('Popup diblokir','Izinkan popup browser untuk mencetak.','error');w.document.open();w.document.write(html);w.document.close();setTimeout(()=>{w.focus();w.print();},350);
  }
  ...
  function studentProfilePrintHtml(s,rows){
    const logo=state.appLogoData||state.appLogo||'assets/logo-ecakep.svg';
    const total=rows.reduce((n,v)=>n+Number(v.pointPotongan||pointFor(v.klasifikasi)),0);
    const wali=getHomeroomTeacher(s.kelas);
    const photo=state.photoCache.get(photoKey('student',s.id))||'';
    return `<!doctype html><html><head><meta charset="utf-8"><title>Profil Siswa - ${escAttr(s.nama)}</title><style>@page{size:A4 portrait;margin:12mm}body{font-family:Roboto,Helvetica,Arial,sans-serif;color:#172033;margin:0}.head{display:flex;align-items:center;gap:12px;border-bottom:3px solid #5b2aa6;padding-bottom:10px}.head img.logo{width:52px;height:52px;object-fit:contain}.head h1{font-size:20px;margin:0}.head p{font-size:10px;margin:3px 0 0;color:#667085}.student{margin-top:16px;padding:14px;border:1px solid #dbe2ec;border-radius:10px;display:flex;gap:16px}.student-photo{width:82px;height:82px;object-fit:cover;border-radius:10px;border:1px solid #dbe2ec}.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px 20px;flex:1}.grid div{font-size:10px}.grid b{font-size:12px}.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:14px 0}.stat{border:1px solid #dbe2ec;padding:10px;border-radius:9px}.stat span{display:block;font-size:8px;color:#667085}.stat b{font-size:17px}.stat strong{color:#5b2aa6}table{width:100%;border-collapse:collapse;font-size:9px}th,td{border:1px solid #b8c0cc;padding:6px;text-align:left}th{background:#e9ddff;font-weight:bold}.foot{margin-top:16px;font-size:8px;color:#667085}.sign{margin-top:32px;display:flex;justify-content:flex-end;text-align:center}.sign>div{width:220px}</style></head><body><div class="head"><img class="logo" src="${escAttr(logo)}"><div><h1>E-CAKEP</h1><p>CATATAN KEDISIPLINAN • PROFIL SISWA</p></div></div><div class="student">${photo?`<img class="student-photo" src="${escAttr(photo)}" alt="Foto siswa">`:''}<div class="grid"><div>Nama<br><b>${escHtml(s.nama)}</b></div><div>NISN<br><b>${escHtml(s.nisn)}</b></div><div>Jenis Kelamin<br><b>${escHtml(s.jenisKelamin)}</b></div><div>Kelas<br><b>${escHtml(s.kelas)}</b></div><div>Wali Kelas<br><b>${wali?`Wali Kelas ${escHtml(wali.nama)}`:'Belum ditentukan'}</b></div></div></div><div class="stats"><div class="stat"><span>Poin Saat Ini</span><b><strong>${normalizePoint(s.poin)}</strong> / 100</b></div><div class="stat"><span>Total Pelanggaran</span><b>${rows.length}</b></div><div class="stat"><span>Total Potongan</span><b>${total} poin</b></div></div><h3>Riwayat Pelanggaran</h3>${rows.length?`<table><thead><tr><th>Tanggal</th><th>Guru</th><th>Klasifikasi</th><th>Jenis Pelanggaran</th><th>Potongan</th><th>Sisa Poin</th></tr></thead><tbody>${rows.map(v=>`<tr><td>${escHtml(displayDate(v.tanggal))}</td><td>${escHtml(v.guru)}</td><td>${escHtml(v.klasifikasi)}</td><td>${escHtml(v.jenisPelanggaran)}</td><td>-${Number(v.pointPotongan||pointFor(v.klasifikasi))}</td><td>${escHtml(v.sisaPoin??'-')}</td></tr>`).join('')}</tbody></table>`:'<p>Belum ada riwayat pelanggaran.</p>'}<div class="foot">Dicetak ${escHtml(new Intl.DateTimeFormat('id-ID',{dateStyle:'full',timeStyle:'short'}).format(new Date()))}. Dokumen ini merupakan ringkasan catatan kedisiplinan siswa.</div><div class="sign"><div>Wali Kelas ${escHtml(s.kelas)}<br><br><br>(${escHtml(wali?.nama||'................................')})</div></div></body></html>`;
  }
  ...
  async function importTeachersExcel(e){ const file=e.target.files?.[0];if(!file)return;try{const parsed=await readExcelRows(file);if(!parsed.length)throw new Error('File Excel kosong.');const headers=parsed[0].map(normalizeHeader);const idx={nama:headers.indexOf('nama'),alamat:headers.indexOf('alamat'),jabatan:headers.indexOf('jabatan'),waliKelas:headers.indexOf('walikelas')};if([idx.nama,idx.alamat,idx.jabatan].some(i=>i<0))throw new Error('Header wajib: Nama, Alamat, Jabatan.');let added=0,updated=0,skipped=0;const changed=[];for(const row of parsed.slice(1)){const nama=String(row[idx.nama]||'').trim(),alamat=String(row[idx.alamat]||'').trim(),jabatan=String(row[idx.jabatan]||'').trim(),waliKelas=idx.waliKelas>=0?String(row[idx.waliKelas]||'').trim():'';if(!nama||!alamat||!jabatan){skipped++;continue;}const existing=state.teachers.find(t=>String(t.nama).trim().toLowerCase()===nama.toLowerCase());const item={id:existing?.id||uid(),nama,alamat,jabatan,waliKelas,updatedAt:new Date().toISOString()};if(existing){Object.assign(existing,item);updated++;}else{state.teachers.push(item);added++;}changed.push(item);}state.teachers.sort(sortByName);save(STORAGE.teachers,state.teachers);refreshAll();toast('Impor guru selesai',`${added} ditambah, ${updated} diperbarui${skipped?`, ${skipped} dilewati`:''}.`,'success');if(state.server.configured&&changed.length)await pushAction('bulkSaveTeachers',{data:changed},true);}catch(err){toast('Impor guru gagal',err.message||'File Excel tidak dapat dibaca.','error');}finally{e.target.value='';} }
  function downloadTeacherTemplate(){ writeExcelFile('template_data_guru_ecakep.xlsx',[['Nama','Alamat','Jabatan','Wali Kelas'],['Budi Santoso','Jl. Pendidikan No. 1','Guru','8A'],['Siti Aminah','Jl. Sekolah No. 2','Guru BK','']],'Data Guru'); }
  ...
  async function fileToLogoJpeg(file){const img=await new Promise((resolve,reject)=>{const i=new Image(),url=URL.createObjectURL(file);i.onload=()=>{URL.revokeObjectURL(url);resolve(i)};i.onerror=()=>{URL.revokeObjectURL(url);reject(new Error('Logo tidak dapat dibaca.'))};i.src=url;});const max=800,scale=Math.min(1,max/Math.max(img.width,img.height)),w=Math.max(1,Math.round(img.width*scale)),h=Math.max(1,Math.round(img.height*scale));const c=document.createElement('canvas');c.width=max;c.height=max;const ctx=c.getContext('2d');ctx.fillStyle='#ffffff';ctx.fillRect(0,0,max,max);ctx.drawImage(img,(max-w)/2,(max-h)/2,w,h);return c.toDataURL('image/jpeg',0.84);}
  ...
  function assertBackend(json){
    ...
    if(Number(json.apiVersion||0)!==API_VERSION) throw new Error(`Backend belum E-CAKEP v3.4. Ditemukan API ${json.apiVersion||0}; dibutuhkan API ${API_VERSION}. Deploy ulang Code.gs v3.4.`);
    return json;
  }
